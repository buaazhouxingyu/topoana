...
void topoana::checkInput()
{
  ...
  if(m_strgTpOfRawIptTopoDat=="AOI")
    ...
    {
      cout<<"Storage type of input raw topology truth information: multiple scalar integer and double-precision numbers."<<endl<<endl;
    }

  if(m_tbrNmOfNps!="nMCGen")
    {
      if(m_strgTpOfRawIptTopoDat=="AOI"||m_strgTpOfRawIptTopoDat=="MSI"||m_strgTpOfRawIptTopoDat=="MSF"||m_strgTpOfRawIptTopoDat=="MSD"||m_strgTpOfRawIptTopoDat=="MSID") cout<<"TBranch name of the number of particles: "<<m_tbrNmOfNps<<endl<<endl;
      ...
      for(unsigned int i=0;i<m_vPid_compCascDcyBrP.size();i++)
        {
          ...
          writeInputRecInfo(m_vTypeOfTagRec_compCascDcyBrP[i], m_vTBrNmOfTagRec_compCascDcyBrP[i], m_vTBrNmOfNRec_compCascDcyBrP[i], "particle");
      ...
      for(unsigned int i=0;i<m_vPid_sigP.size();i++)
        {
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigP[i], m_vTBrNmOfTagRec_sigP[i], m_vTBrNmOfNRec_sigP[i], "particle");
          cout<<endl;
        }
      cout<<endl;
      ...
      for(unsigned int i=0;i<m_vSigDcyBr.size();i++)
        {
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigDcyBr[i], m_vTBrNmOfTagRec_sigDcyBr[i], m_vTBrNmOfNRec_sigDcyBr[i], "mother particle of the decay branch");
          cout<<endl;
        }
    }
      ...
      for(unsigned int i=0;i<m_vSigIncDcyBr.size();i++)
        {
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigIncDcyBr[i], m_vTBrNmOfTagRec_sigIncDcyBr[i], m_vTBrNmOfNRec_sigIncDcyBr[i], "mother particle of the inclusive decay branch");
          cout<<endl;
        }
    }
  ...
  else
    {
      ...
      for(unsigned int i=0;i<m_vSigCascDcyBr.size();i++)
        { 
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigCascDcyBr[i], m_vTBrNmOfTagRec_sigCascDcyBr[i], m_vTBrNmOfNRec_sigCascDcyBr[i], "mother particle of the cascade decay branch");
          cout<<endl;
        }
  ...
  else
    {
      ...
      for(unsigned int i=0;i<m_vSigIncCascDcyBr.size();i++)
        { 
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigIncCascDcyBr[i], m_vTBrNmOfTagRec_sigIncCascDcyBr[i], m_vTBrNmOfNRec_sigIncCascDcyBr[i], "mother particle of the inclusive cascade decay branch");
          cout<<endl;
        }
      ...
      for(unsigned int i=0;i<m_vSigIRADcyBr.size();i++)
        {
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigIRADcyBr[i], m_vTBrNmOfTagRec_sigIRADcyBr[i], m_vTBrNmOfNRec_sigIRADcyBr[i], "mother particle of the intermediate-resonance-allowed decay branch");
          cout<<endl;
        }
    }
  ...
  else
    {
      ...
      for(unsigned int i=0;i<m_vSigIncOrIRACascDcyBr.size();i++)
        { 
          ...
          writeInputRecInfo(m_vTypeOfTagRec_sigIncOrIRACascDcyBr[i], m_vTBrNmOfTagRec_sigIncOrIRACascDcyBr[i], m_vTBrNmOfNRec_sigIncOrIRACascDcyBr[i], "mother particle of the inclusive or intermediate-resonance-allowed cascade decay branch");
          cout<<endl;
        }
  ...
  if(m_convtMSIFDIntoAOI==true)
    {
      if(m_strgTpOfRawIptTopoDat=="MSI"||m_strgTpOfRawIptTopoDat=="MSF"||m_strgTpOfRawIptTopoDat=="MSD"||m_strgTpOfRawIptTopoDat=="MSID")
        {
          cout<<"MSI/MSF/MSD/MSID input tbranches will be converted into AOI output tbranches."<<endl;
        ...
        {
          cerr<<"Warning: the setting with the item prompted with \"% Convert MSI/MSF/MSD/MSID input tbranches into AOI output tbranches (Two options: Y and N. Default: N)\" does not work for the current cases of AOI/VOI input tbranches."<<endl<<endl;
        }
    }
  else
    {
      if(m_vbsLevStdOut==true) cout<<"MSI/MSF/MSD/MSID input tbranches will not be converted into AOI output tbranches (default)."<<endl<<endl;
    }
