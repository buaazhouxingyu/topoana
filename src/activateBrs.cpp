          ...
          if(vICc[i]==0)
            {
              ...
              if(m_flatArrayTBrsLocally==true) nMax[i]=tr->GetMaximum(specifier1);
              if(m_flatArrayTBrsLocally==true) if(nMax[i]>nMax_Max) nMax_Max=nMax[i];
            }
          else
            {
              sprintf(specifier1, "n%s", nm);
              ...
              if(m_flatArrayTBrsLocally==true) nMax[i]=tr->GetMaximum(specifier1);
              if(m_flatArrayTBrsLocally==true) if(nMax[i]>nMax_Max) nMax_Max=nMax[i];
              if(m_flatArrayTBrsLocally==true) nCcMax[i]=tr->GetMaximum(specifier3);
              if(m_flatArrayTBrsLocally==true) if(nCcMax[i]>nCcMax_Max) nCcMax_Max=nCcMax[i];
            }
        }   
      else 
        {
