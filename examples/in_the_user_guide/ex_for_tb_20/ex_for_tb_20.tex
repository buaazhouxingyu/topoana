...
\tablecaption{Signal inclusive cascade decay branches.}
\tableheaderP{rowNo & \thead{signal inclusive cascade decay branch} & \topoTags{iSigIncCascDcyBr & }nCase & nCCase \\}

% \rn = 1
\rn & \makecell[l]{ $ 
\bar{B}^{0} \rightarrow D^{*+} +anything ,
D^{*+} \rightarrow \pi^{+} D^{0} 
$ } & \topoTags{0 & }28367 & 28367 \EOL

% \rn = 2
\rn & \makecell[l]{ $ 
B^{0} \rightarrow K_{S}^{0} J/\psi ,
K_{S}^{0} \rightarrow \pi^{+} \pi^{-} ,
J/\psi \rightarrow \mu^{+} +anything 
$ } & \topoTags{1 & }1 & 28368 \\ \hline
