          ...
          if(m_strgTpOfRawIptTopoDat=="MSD")
            {
              for(unsigned int k=0;k<vTypeOfTagRec2.size();k++)
                if(vTBrNmOfTagRec2[k]==vTBrNmOfTagRec1[j]) Tagrecsd2[k]=Tagrecsd1[j];
            }
          else
            {
              for(unsigned int k=0;k<m_vPid_compDcyBrP.size();k++)
                if(vTBrNmOfTagRec2[k]==vTBrNmOfTagRec1[j]) Tagrecsi2[k]=Tagrecsi1[j];
            }
        }
      else if(vTypeOfTagRec1[j]=="C"||vTypeOfTagRec1[j]=="N"||vTypeOfTagRec1[j]=="!N"||vTypeOfTagRec1[j]=="P")
        {
          for(unsigned int k=0;k<m_vPid_compDcyBrP.size();k++)
            {
              if(vTBrNmOfTagRec2[k]==vTBrNmOfTagRec1[j])
                {
                  if(vTBrNmOfNRec2[k]==vTBrNmOfNRec1[j])
                  ...
                  else
                    {
                      ...
                      cerr<<"Infor: This is an apparent mistake. Are there any typos in these TBranch names? Please check them."<<endl<<endl;
                      exit(-1);
                    }
                }
